...
;
; CALLING SEQUENCE:
;   parms = MPFIT_MR(MYFUNCT, start_parms, FUNCTARGS=fcnargs, NFEV=nfev,
...
; DESCRIPTION:
;
;  MPFIT_MR uses the Levenberg-Marquardt technique to solve the
;  least-squares problem.  In its typical use, MPFIT_MR will be used to
;  fit a user-supplied function (the "model") to user-supplied data
;  points (the "data") by adjusting a set of parameters.  MPFIT_MR is
;  based upon MINPACK-1 (LMDIF.F) by More' and collaborators.
;
;  For example, a researcher may think that a set of observed data
;  points is best modelled with a Gaussian curve.  A Gaussian curve is
;  parameterized by its mean, standard deviation and normalization.
;  MPFIT_MR will, within certain constraints, find the set of parameters
...
;  recommended to use the convenience functions MPFIT_MRFUN and
;  MPFIT_MREXPR, which are driver functions that calculate the deviates
;  for you.  If ERR are the 1-sigma uncertainties in Y, then
;
;    TOTAL( DEVIATES^2 ) 
;
;  will be the total chi-squared value.  MPFIT_MR will minimize the
;  chi-square value.  The values of X, Y and ERR are passed through
;  MPFIT_MR to the user-supplied function via the FUNCTARGS keyword.
;
;  Simple constraints can be placed on parameter values by using the
;  PARINFO keyword to MPFIT_MR.  See below for a description of this
;  keyword.
;
;  MPFIT_MR does not perform more general optimization tasks.  See TNMIN
;  instead.  MPFIT_MR is customized, based on MINPACK-1, to the
...
;  MYFUNCT by using the FUNCTARGS keyword to MPFIT_MR.  Use MPFIT_MRFUN and
...
; 
;  In the search for the best-fit solution, MPFIT_MR by default
...
;  MPFIT_MR to calculate the derivatives numerically, and so
...
;
;  The behavior of MPFIT_MR can be modified with respect to each
...
;  passed as a keyword parameter to MPFIT_MR.
;
...
;              MPFIT_MR, but are passed on to MYFUNCT for evaluation.
;  
...
;                fitting code of MPFIT_MR does not use this tag in any
...
;
;  The normal mode of operation for MPFIT_MR is for the user to pass a
;  function name, and MPFIT_MR will call the user function multiple times
;  as it iterates toward a solution.
;
;  Some user functions are particularly hard to compute using the
;  standard model of MPFIT_MR.  Usually these are functions that depend
;  on a large amount of external data, and so it is not feasible, or
;  at least highly impractical, to have MPFIT_MR call it.  In those cases
...
;  and re-call MPFIT_MR.  The user may refrain from calling MPFIT_MR
;  further; as usual, STATUS will indicate when the solution has
;  converged and no more iterations are required.
;
;  Because MPFIT_MR must maintain its own data structures between calls,
...
;           by MPFIT_MR.  The matrix is NxN where N is the number of
...
;
;           If NOCOVAR is set or MPFIT_MR terminated abnormally, then
...
;                   array.
;
;   EXTERNAL_STATE - a named variable to store MPFIT_MR-related state
;                    information between iterations (used in input and
;                    output to MPFIT_MR).  The user must not manipulate
...
;              MPFIT_MR will run faster, but it will be more prone to
...
;              iteration of the MPFIT_MR routine.  ITERPROC is always
...
;              between -15 and -1 (see MPFIT_MR_ERROR common block
...
;             modified during a call to MPFIT_MR.
;
...
;             description of MPFIT_MR_ERROR common below).  If either
...
;   fa = {X:x, Y:y, ERR:err}
;   p = mpfit_mr('MYFUNCT', p0, functargs=fa)
;
...
; COMMON BLOCKS:
;
;   COMMON MPFIT_MR_ERROR, ERROR_CODE
;
...
;     procedure or function returns, MPFIT_MR checks the value of this
...
;
;   COMMON MPFIT_MR_PROFILE
;   COMMON MPFIT_MR_MACHAR
;   COMMON MPFIT_MR_CONFIG
;
;     These are undocumented common blocks are used internally by
;     MPFIT_MR and may change in future implementations.
;
; THEORY OF OPERATION:
;
...
;   when it is not found by finite differences (see MPFIT_MR_FDJAC2,
...
;   The routine MPFIT_MR_QRFAC provides the QR factorization of h, with
;   pivoting, and MPFIT_MR_QRSOLV provides the solution for dx.
;   
; REFERENCES:
;
...
;   Added keyboard stop to MPFIT_MR_DEFITER, 28 Feb 1999 CM
;   Cosmetic documentation changes, 14 May 1999 CM
;   IDL optimizations for speed & FASTNORM keyword, 15 May 1999 CM
;   Tried a faster version of mpfit_mr_enorm, 30 May 1999 CM
;   Changed web address to cow.physics.wisc.edu, 14 Jun 1999 CM
;   Found malformation of FDJAC in MPFIT_MR for 1 parm, 03 Aug 1999 CM
;   Factored out user-function call into MPFIT_MR_CALL.  It is possible,
;     but currently disabled, to call procedures.  The calling format
;     is similar to CURVEFIT, 25 Sep 1999, CM
;   Slightly changed mpfit_mr_tie to be less intrusive, 25 Sep 1999, CM
;   Fixed some bugs associated with tied parameters in mpfit_mr_fdjac, 25
;     Sep 1999, CM
...
;   Split out MPFIT_MR_RESETPROF to aid in profiling, 03 Nov 1999, CM
...
;   Made sure that !err was set to 0 in MPFIT_MR_DEFITER, 10 Jan 2000, CM
;   Fixed small inconsistency in setting of QANYLIM, 28 Jan 2000, CM
;   Added PARINFO field RELSTEP, 28 Jan 2000, CM
;   Converted to MPFIT_MR_ERROR common block for indicating error
;     conditions, 28 Jan 2000, CM
;   Corrected scope of MPFIT_MR_ERROR common block, CM, 07 Mar 2000
;   Minor speed improvement in MPFIT_MR_ENORM, CM 26 Mar 2000
;   Corrected case where ITERPROC changed parameter values and
;     parameter values were TIED, CM 26 Mar 2000
;   Changed MPFIT_MR_CALL to modify NFEV automatically, and to support
;     user procedures more, CM 26 Mar 2000
;   Copying permission terms have been liberalized, 26 Mar 2000, CM
;   Catch zero value of zero a(j,lj) in MPFIT_MR_QRFAC, 20 Jul 2000, CM
;      (thanks to David Schlegel <schlegel@astro.princeton.edu>)
;   MPFIT_MR_SETMACHAR is called only once at init; only one common block
;     is created (MPFIT_MR_MACHAR); it is now a structure; removed almost
...
;   Correct bug in MPFIT_MR_LMPAR of singularity handling, which might
...
;
;  $Id: mpfit_mr.pro,v 1.2 2004/06/03 06:54:51 observer Exp $
;-
...
pro mpfit_mr_dummy
  ...
  FORWARD_FUNCTION mpfit_mr_fdjac2, mpfit_mr_enorm, mpfit_mr_lmpar, mpfit_mr_covar, $
    mpfit_mr, mpfit_mr_call

  COMMON mpfit_mr_error, error_code  ;; For error passing to user function
  COMMON mpfit_mr_config, mpconfig   ;; For internal error configrations
end

;; Reset profiling registers for another run.  By default, and when
;; uncommented, the profiling registers simply accumulate.

pro mpfit_mr_resetprof
  common mpfit_mr_profile, mpfit_mr_profile_vals

  mpfit_mr_profile_vals = { status: 1L, fdjac2: 0D, lmpar: 0D, mpfit_mr: 0D, $
...
pro mpfit_mr_setmachar, double=isdouble
  common mpfit_mr_profile, profvals
  if n_elements(profvals) EQ 0 then mpfit_mr_resetprof

  common mpfit_mr_machar, mpfit_mr_machar_vals
  ...
  mpfit_mr_machar_vals = {machep: dmachep, maxnum: dmaxnum, minnum: dminnum, $
...
;; derivatives or not.
function mpfit_mr_call, fcn, x, fjac, _EXTRA=extra

  on_error, 2
  common mpfit_mr_config, mpconfig

  if keyword_set(mpconfig.qanytied) then mpfit_mr_tie, x, mpconfig.ptied
...
function mpfit_mr_fdjac2, fcn, x, fvec, step, ulimited, ulimit, dside, $
                 iflag=iflag, epsfcn=epsfcn, autoderiv=autoderiv, $
                 FUNCTARGS=fcnargs, xall=xall, ifree=ifree, dstep=dstep,$
                          _EXTRA=extra

  common mpfit_mr_machar, machvals
  common mpfit_mr_profile, profvals
  common mpfit_mr_error, mperr

  if keyword_set(fcnargs) and keyword_set(extra) then $
    ex=create_struct('fcnargs',fcnargs, extra) $
  else if keyword_set(fcnargs) then ex=fcnargs $
  else if keyword_set(extra) then ex=extra
...
;  prof_start = systime(1)
  ...
  if NOT keyword_set(autoderiv) then begin
      ...
      fp = mpfit_mr_call(fcn, xall, fjac, _EXTRA=ex)
      iflag = mperr
...
;      profvals.fdjac2 = profvals.fdjac2 + (systime(1) - prof_start)
  ...
  for j=0L, n-1 do begin
      ...
      mperr = 0
      fp = mpfit_mr_call(fcn, xp, _EXTRA=ex)
      ...
      if abs(dside(j)) LE 1 then begin
          ...
          mperr = 0
          fm = mpfit_mr_call(fcn, xp, _EXTRA=ex)
...
function mpfit_mr_enorm, vec, nosqr=nosqr,_EXTRA=extra
...
; Very simple-minded sum-of-squares
  ...
  ;if keyword_set(nosqr) then return, sqrt(total(vec))
...
;    aa = [[9.,2.,6.],[4.,8.,7.]]
;    mpfit_mr_qrfac, aa, aapvt, rdiag, aanorm
;     IDL> print, aa
...
; explicitly, and MPFIT_MR does not.

pro mpfit_mr_qrfac, a, ipvt, rdiag, acnorm, pivot=pivot
...
;  prof_start = systime(1)
  ...
  for j = 0L, minmn-1 do begin
      ...
      ;; exchange occurs later, within the body of MPFIT_MR, after the
          ...
          for k=j+1, n-1 do begin
              ...
              if keyword_set(pivot) AND rdiag(k) NE 0 then begin
                  ...
                  if 0.05D*temp*temp LE MACHEP0 then begin
                      rdiag(k) = mpfit_mr_enorm(a(j+1:*,lk))
                      wa(k) = rdiag(k)
                  endif
              endif
          endfor
...
;     burton s. garbow, kenneth e. hillstrom, jorge j. more
;
pro mpfit_mr_qrsolv, r, ipvt, my_diag, qtb, x, sdiag
...
;  prof_start = systime(1)
  ...
  for j = 0L, n-1 do begin
      l = ipvt(j)
      if my_diag(l) EQ 0 then goto, STORE_RESTORE
      sdiag(j:*) = 0
      sdiag(j) = my_diag(l)
...
;     burton s. garbow, kenneth e. hillstrom, jorge j. more
;
function mpfit_mr_lmpar, r, ipvt, my_diag, qtb, delta, x, sdiag, par=par
...
;  prof_start = systime(1)
  ...
  if nsing GE n then begin
      wa1 = my_diag(ipvt)*wa2(ipvt)/dxnorm
  ...
  for j=0, n-1 do begin
      ...
      wa1(j) = sum/my_diag(ipvt(j))
  endfor
  gnorm = mpfit_mr_enorm(wa1)
  paru  = gnorm/delta
  ...
  temp = sqrt(par)
  wa1 = temp * my_diag
  mpfit_mr_qrsolv, r, ipvt, wa1, qtb, x, sdiag
  wa2 = my_diag*x
  dxnorm = mpfit_mr_enorm(wa2)
  temp = fp
  fp = dxnorm - delta
...
pro mpfit_mr_defiter, fcn, x, iter, fnorm, FUNCTARGS=fcnargs, $
                   quiet=quiet, iterstop=iterstop, iterkeybyte=iterkeybyte, $
                   parinfo=parinfo, $
                   format=fmt, pformat=pformat, dof=dof0, iterargs=iterargs,$
                      _EXTRA=extra
  ...
  if n_params() EQ 3 then begin
      fvec = mpfit_mr_call(fcn, x, _EXTRA=extra)
      fnorm = mpfit_mr_enorm(fvec)^2
  endif
...
pro mpfit_mr_parinfo, parinfo, tnames, tag, values, default=def, status=status, $
                   n_param=n
...
;
;     **********
function mpfit_mr_covar, rr, ipvt, tol=tol
...
;
;     **********
function mpfit_mr, fcn, xall, FUNCTARGS=fcnargs, SCALE_FCN=scalfcn, $
                ...
                my_diag=my_diag, rescale=rescale, autoderivative=autoderiv, $
                ...
                external_fvec=efvec, external_fjac=efjac, _EXTRA=extra
  ...
  if n_params() EQ 0 then begin
      message, "USAGE: PARMS = MPFIT_MR('MYFUNCT', START_PARAMS, ... )", /info
      return, !values.d_nan
  endif
  ...
  if n_elements(iterproc) EQ 0 then iterproc = 'MPFIT_MR_DEFITER'
  ...
  ;; a set of keywords to pass to MPFIT_MR_DEFITER.
  if strupcase(iterproc) EQ 'MPFIT_MR_DEFITER' AND n_elements(iterargs) EQ 0 $
  ...
  if n_elements(xall) EQ 0 then begin
      mpfit_mr_parinfo, parinfo, tagnames, 'VALUE', xall, status=status
  ...
  ;; TIED parameters?
  mpfit_mr_parinfo, parinfo, tagnames, 'TIED', ptied, default='', n=npar
  ...
  ;; FIXED parameters ?
  mpfit_mr_parinfo, parinfo, tagnames, 'FIXED', pfixed, default=0, n=npar
  pfixed = pfixed EQ 1
  pfixed = pfixed OR (ptied NE '');; Tied parameters are also effectively fixed
  
  ;; Finite differencing step, absolute and relative, and sidedness of deriv.
  mpfit_mr_parinfo, parinfo, tagnames, 'STEP',     step, default=zero, n=npar
  mpfit_mr_parinfo, parinfo, tagnames, 'RELSTEP', dstep, default=zero, n=npar
  mpfit_mr_parinfo, parinfo, tagnames, 'MPSIDE',  dside, default=0,    n=npar

  ;; Maximum and minimum steps allowed to be taken in one iteration
  mpfit_mr_parinfo, parinfo, tagnames, 'MPMAXSTEP', maxstep, default=zero, n=npar
  mpfit_mr_parinfo, parinfo, tagnames, 'MPMINSTEP', minstep, default=zero, n=npar
  ...
  ;; LIMITED parameters ?
  mpfit_mr_parinfo, parinfo, tagnames, 'LIMITED', limited, status=st1
  mpfit_mr_parinfo, parinfo, tagnames, 'LIMITS',  limits,  status=st2
  ...
  if keyword_set(rescale) then begin
      ...
      if n_elements(my_diag) LT n then goto, TERMINATE
      wh = where(my_diag LE 0, ct)
  ...
  if NOT isext then begin
      ...
      fvec = mpfit_mr_call(fcn, xnew, _EXTRA=ex)
      iflag = mperr
  ...
  catch_msg = 'calling MPFIT_MR_SETMACHAR'
  sz = size(fvec(0))
  isdouble = (sz(sz(0)+1) EQ 5)
  
  mpfit_mr_setmachar, double=isdouble
...
;  prof_start = systime(1)
  ...
  if n_e(extra) gt 0 then begin
      w=where(tag_names(extra) eq 'USE_COVAR',c)
      if c gt 0 then nosqr=extra.use_covar else nosqr=0
  endif else nosqr=0
  fnorm = mpfit_mr_enorm(fvec, nosqr=nosqr,_EXTRA=extra)
  ...
  xnew(ifree) = x
  if qanytied then mpfit_mr_tie, xnew, ptied
  ...
  if nprint GT 0 AND iterproc NE '' then begin
      ...
      if (iter-1) MOD nprint EQ 0 then begin
          ...
          call_procedure, iterproc, fcn, xnew, iter, fnorm^2, $
            ...
            dof=dof, iterargs=iterargs,_EXTRA=extra
          iflag = mperr
  ...
  iflag = 2
  if NOT isext then begin
      catch_msg = 'calling MPFIT_MR_FDJAC2'
      fjac = mpfit_mr_fdjac2(fcn, x, fvec, step, qulim, ulim, dside, $
                          iflag=iflag, epsfcn=epsfcn, $
                          autoderiv=autoderiv, dstep=dstep, $
                          FUNCTARGS=fcnargs, ifree=ifree, xall=xnew, _EXTRA=extra)
      if iflag LT 0 then begin
  ...
  catch_msg = 'calling MPFIT_MR_QRFAC'
  mpfit_mr_qrfac, fjac, ipvt, wa1, wa2, /pivot
  ...
  if (iter EQ 1) then begin
      ...
      if NOT keyword_set(rescale) OR (n_elements(my_diag) LT n) then begin
          my_diag = wa2
          wh = where (my_diag EQ 0, ct)
          if ct GT 0 then my_diag(wh) = one
      endif
      
      ;; On the first iteration, calculate the norm of the scaled x
      ;; and initialize the step bound delta 
      wa3 = my_diag * x
      xnorm = mpfit_mr_enorm(wa3)
      delta = factor*xnorm
  ...
  catch_msg = 'calculating LM parameter (MPFIT_MR_LMPAR)'
  par = mpfit_mr_lmpar(fjac, ipvt, my_diag, qtf, delta, wa1, wa2, par=par)
  ...
  mperr = 0
  catch_msg = 'calling '+fcn
  wa4 = mpfit_mr_call(fcn, xnew, _EXTRA=ex)
  iflag = mperr
  if iflag LT 0 then begin
  ...
  RESUME_FIT:
  fnorm1 = mpfit_mr_enorm(wa4,nosqr=nosqr, _EXTRA=extra)
  ...
  ;; taken
  temp1 = mpfit_mr_enorm(alpha*wa3)/fnorm
    ...
    AND n_elements(fjac) GT 0 AND n_elements(ipvt) GT 0 then begin
      ...
      if n GT 0 AND sz(0) GT 1 AND sz(1) GE n AND sz(2) GE n $
        AND n_elements(ipvt) GE n then begin
          ...
          if n EQ 1 then $
            cv = mpfit_mr_covar(reform([fjac(0,0)],1,1), ipvt(0)) $
          else $
            cv = mpfit_mr_covar(fjac(0:n-1,0:n-1), ipvt(0:n-1))
...
;  profvals.mpfit_mr = profvals.mpfit_mr + (systime(1) - prof_start)
  ...
  catch_msg = 'saving MPFIT_MR state'

  ;; Names of variables to save
  varlist = ['alpha', 'delta', 'my_diag', 'dwarf', 'factor', 'fnorm', $
  ...
  if execute(cmd) NE 1 then $
    message, 'ERROR: could not save MPFIT_MR state'
