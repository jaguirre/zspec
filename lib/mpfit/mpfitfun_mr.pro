...
;
; CALLING SEQUENCE:
;   parms = MPFITFUN_MR(MYFUNCT, X, Y, ERR, start_params, ...)
;
; DESCRIPTION:
;
;  MPFITFUN_MR fits a user-supplied model -- in the form of an IDL
;  function -- to a set of user-supplied data.  MPFITFUN_MR calls
;  MPFIT, the MINPACK-1 least-squares minimizer, to do the main
;  work.
;
;  Given the data and their uncertainties, MPFITFUN_MR finds the best set
...
;
;  MPFITFUN_MR carefully avoids passing large arrays where possible to
;  improve performance.
;
...
;  AUTODERIVATIVE is accepted by MPFITFUN_MR and passed directly to
...
;   p = mpfitfun_mr('GAUSS1', x, y, sy, p0)    ; Fit a function
;   print, p
;
...
;
;  $Id: mpfitfun_mr.pro,v 1.2 2004/06/03 06:54:51 observer Exp $
;-
...
FORWARD_FUNCTION mpfitfun_mr_eval, mpfitfun_mr, mpfit_mr

; This is the call-back function for MPFIT.  It evaluates the
; function, subtracts the data, and returns the residuals.
function mpfitfun_mr_eval, p, dp, _EXTRA=extra

  common mpfitfun_mr_common, fcn, x, y, err, wts, f, fcnargs, data_covar

  ;; The function is evaluated here.  There are four choices,
  ;; depending on whether (a) FUNCTARGS was passed to MPFITFUN_MR, which
  ...
  if n_e(extra) eq 0 then use_covar=0 else begin
      tags=tag_names(extra)
      w=where(tags eq 'USE_COVAR', count)
      if count eq 0 then use_covar=0 else use_covar=extra.use_covar 
  endelse

  ;; Compute the deviates, applying either errors or weights

  if use_covar then begin
      
      inv_cov=invert(data_covar)
      inv_cov=.5*(inv_cov+transpose(inv_cov))
      evals=eigenql(inv_cov,/double,eigenvectors=evects)
      result=diag(sqrt(evals))#transpose(evects)#(y-f)
                                ;result=(y-f)*(invert(data_covar)#(y-f))
      ;signs=result/abs(result)
      ;result=signs*sqrt(abs(result))
...
pro mpfitfun_mr_cash, resid, dresid
  common mpfitfun_mr_common, fcn, x, y, err, wts, f, fcnargs
...
function mpfitfun_mr, fcn, x, y, err, p, WEIGHTS=wts, FUNCTARGS=fa, $
                   ...
                   quiet=quiet, ERRMSG=errmsg, data_covar=data_covar,$
                      _EXTRA=extra

  status = 0L
  errmsg = ''
  use_covar=keyword_set(data_covar)
  ...
  if catcherror NE 0 then begin
      ...
      message, 'ERROR: the required function MPFIT_MR must be in your IDL path', /info
      return, !values.d_nan
  endif
  if mpfit_mr(/query) NE 1 then goto, MPFIT_NOTFOUND
  catch, /cancel
  if keyword_set(query) then return, 1

  if n_params() EQ 0 then begin
      message, "USAGE: PARMS = MPFITFUN_MR('MYFUNCT', X, Y, ERR, "+ $
        "START_PARAMS, ... )", /info
      return, !values.d_nan
  ...
  if keyword_set(cash) then begin
      scalfcn = 'mpfitfun_mr_cash'
  endif

  ;; Use common block to pass data back and forth
  common mpfitfun_mr_common, fc, xc, yc, ec, wc, mc, ac, dc
  ...
  if keyword_set(data_covar) then dc=data_covar
  ...
  if n_e(extra) eq 0 then ex=create_struct('USE_COVAR', use_covar) $
  else  ex=create_struct('USE_COVAR', use_covar, extra)

  result = mpfit_mr('mpfitfun_mr_eval', p, SCALE_FCN=scalfcn, $
                 parinfo=parinfo, STATUS=status, nfev=nfev, BESTNORM=bestnorm,$
                 covar=covar, perror=perror, $
                 niter=niter, nfree=nfree, npegged=npegged, dof=dof, $
                 ERRMSG=errmsg, quiet=quiet, _EXTRA=ex)
