; $Id: tvaxis.pro,v 1.3 2004/06/07 17:08:05 stoverp Exp $
; $Log: tvaxis.pro,v $
; Revision 1.3  2004/06/07 17:08:05  stoverp
; Fixing apparent discrepancy with presence of _EXTRA keyword.
;
; Revision 1.2  2003/12/18 22:47:32  stoverp
; Added xminor/yminor keywords.
;
; Revision 1.1  2003/11/14 20:16:56  stoverp
; Initial import -- SolarSoft functions
;
	PRO TVAXIS,ARRAY,MX,MY,JX,JY,XAXIS=XAXIS,YAXIS=YAXIS,XRANGE=XRANGE,  $
		...
		DISABLE=DISABLE,DATA=DATA,ORIGIN=ORIGIN,SCALE=SCALE,         $
                XMINOR=XMINOR,YMINOR=YMINOR,_EXTRA=_EXTRA
;+
; Project     : SOHO - CDS
;
; Name        : 
...
; 031218 PS - Added support for xminor/yminor keywords
...
;
	IF N_ELEMENTS(XAXIS) EQ 1 THEN BEGIN
		...
		IF N_ELEMENTS(XTICKV) NE 0 THEN COMMAND = COMMAND +	$
			",XTICKV=XTICKV"
		IF N_ELEMENTS(XMINOR) NE 0 THEN COMMAND = COMMAND +	$    ;;; Added
			",XMINOR=XMINOR"                                     ;;; -- PS
                COMMAND = COMMAND + ",_EXTRA=_EXTRA"
