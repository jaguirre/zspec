...
;      WHERE etc.) pointing to elements of a n-dimensional array , an array 
...
;*> Check input
  ...
  if Nindx eq 0 then $
    message,'Usage: index_set=GET_INDX(index_array,dimensions)'
  ...
  if Ndim eq 0 then $
    message,'Usage: index_set=GET_INDX(index_array,dimensions)'
  N=long(dimensions)
  if (size(N))(0) gt 1 then $
    message,'Dimensions of the array must be specified by a vector'
