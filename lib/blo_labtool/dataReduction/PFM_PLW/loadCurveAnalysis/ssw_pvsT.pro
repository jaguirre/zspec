...
path0= '/data1/SPIRE_PFM_SSW/'			       
path = path0+'20031208/'			       


;Rstore the Rstar and Tstar from a file
bolo_restore_rstar_tstar, Rstar, Tstar, path=path, $
        filename='bolo_rstar_tstar_20031208.txt'
        ;filename='RoDel_Hien_20031208.txt'
...
restore, path+'loadcrv_20031208.sav'

nchannel = n_elements((*x[0]).ubolo[*,0])

Tbath=0.251d
common_name_str='_20031208'
...
for channel=0, nchannel-1 do begin
  ...
  if cnt GT 0 then begin
      ...
      plot, T, P*1e12+channel*dp, psym=3, charsize=1.6, title='PFM SSW' $
        ,xtitle='T [K]', ytitle='P [pW]' $
        ,xthick=4, ythick=4, xmargin=[8,3] $
        , xrange=[0.28,0.64], yrange=[0,dp*nchannel*1.6], xstyle=3, ystyle=1 $
        ,/nodata
...
;     xyouts, T[ix2], P[ix2]*1e12+channel*dp+0.01, (*x[0]).ubolo_label[channel] $
      ...
      xyouts, 0.62, channel*dp*1.4+10, (*x[0]).ubolo_label[channel] $
          , charsize=0.6
      xyouts, 0.31, 220, 'P shifted up by 4pW for each channel'
...
psterm , file='ssw_pvsT'+common_name_str+'.ps'


openw, un, 'bolo_rstar_tstar_g0_beta'+common_name_str+'.txt', /get_lun
