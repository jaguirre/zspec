; $Id: mk_html_help.pro,v 1.1 2004/04/16 21:37:27 spirebolo Exp $
;
...
;	Dec 10, 2003, B. Schulz (IPAC) Added linking to routines
;       Jan. 2, 2004, Added the sources to the message text.
;-
;
...
;
 ...
 WHILE (1) DO BEGIN
  ...
  IF (found) THEN BEGIN
   ...
   index = WHERE(strupcase(STRTRIM(header, 2)) EQ 'NAME:', count)	;change 2003/12/10
 ...
 FOR i=0,count-1 DO BEGIN
  ...
  IF (entry.nline GT 0) THEN BEGIN
   ...
   IF (entry.lib NE -1L) THEN BEGIN
    ...
    PRINTF,final_file,'<STRONG>(See '+fname+' in '
    PRINTF,final_file,'<A href="file://'+lname +'" > '+lname +' </A>'	
    PRINTF,final_file,')</STRONG><P>'

   ENDIF ELSE BEGIN

    fname = infiles[entry.indx]
    IF (strict) THEN mhh_strict,fname
    PRINTF,final_file, '<STRONG>(See '
    PRINTF,final_file, '<A href="file://'+fname +'" > '+fname +' </A>'
    PRINTF,final_file, ')</STRONG><P>'
...
PRO mk_html_help, sources, outfile, VERBOSE=verbose, TITLE=title, STRICT=strict
 ...
 IF (count EQ 1) THEN BEGIN
 msg='No IDL files found in: ' + sources
  MESSAGE,/INFO,msg
  RETURN
 ENDIF 
 infiles = infiles[1:*]
