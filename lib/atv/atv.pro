...
;           [,/align] [,/stretch] [,header = header] [,zoom=zoom]
;
; REQUIRED INPUTS:
...
;       zoom:       allows zoom in or out on call to ATV (see lines 149, 150)
;       
; OUTPUTS:
;       None.  
; 
...
;       Requires Craig Markwardts cmps_form.pro routine.
...
state = {                   $
          ...
          active_window_id: 0L, $        ; users active window outside atv
...
nplot = 0
maxplot = 5000
plot_ptr = ptrarr(maxplot+1)  ; The 0th element is not used.
...
; Add extra padding for menu bar, since this is not included in 
...
; event handler for exit events of main draw base.  There is no need to
...
; tweaked and debugged by AJB.  It does not always work exactly the way
...
; Note: do not know why the (-4) is needed below, but it seems
...
; postscript, because if it is we cannot display anything.
...
; If fitsfilename has not been passed to this routine, get filename
...
; Make sure it is not a 1-d spectrum
...
; Make sure it is not a fits table: this would make mrdfits crash
...
; if there is blank space around the image border, keep it black
tv, newdisplay
atv_plotall
...
; header if available.  If there is no astrometry information, set 
...
; coordinate types that we cannot use:
...
; If there is no header, kill the headinfo window and exit this
; routine.
...
; routine arcbar does not recognize color=0, because it uses 
...
; This is a copy of the IDL Astronomy Users Library routine 'arcbar',
...
; arcbar.pro in the pro/astro subdirectory of the IDL Astronomy Users
; Library.
...
endif else  dmin = dmin * state.zoom_factor    ; added by AJB Jan. 2000
...
phpadu = 1.0                    ; do not convert counts to electrons
apr = [state.r]
...
; Before starting up atv, get the users external window id.  We cannot
; use the atv_getwindow routine yet because we have not run atv
; startup.  A subtle issue: atv_resetwindow will not work the first time
; through because xmanager does not get called until the end of this
...
; Allow for zoom on call to atv. This acts just like the zoom widget,
; raising or lowering the magnification by powers of two
    IF (n_elements(zoom) GT 0) THEN BEGIN
       IF (zoom GT 6. OR zoom LT -6.) THEN BEGIN
	  print, 'Zoom factor must be between -6 and 6:'
	  print, 'Image magnification is 2^zoom'
       ENDIF ELSE BEGIN 
...
; Register the widget with xmanager if it is not already registered
if (not(xregistered('atv', /noshow))) then begin
    nb = abs(block - 1)
