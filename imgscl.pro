...
;
	...
	if ~finite(maskvalue) then begin
		valid=where(finite(array), valid_count)
	endif else begin
...
;
	...
	0: Begin
	    image = BytScl(array, Min=min_lvl, Max=max_lvl, Top=top-1B, /nan) + 1B
	End
	1: Begin
	    tmp = 1.0 + (array <max_lvl >min_lvl) - min_lvl
	    tmp = ALog10(Temporary(tmp) >low_clip)
	    image = BytScl(tmp, Top=top-1B, /nan) + 1B
	End
	2: Begin
	;
