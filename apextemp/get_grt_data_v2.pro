...
; Now have to work with the merged data, since Tom has helpfully
; zipped up the individual board data.  But some stuff is only
; available in the previous format.  So ...

files = [file_search('~/data/telescope_tests/20100927/*merged.nc'),$
         file_search('/data/20100927/*merged.nc')]
whnotnull = where(files ne '')
files = files[whnotnull]

nfiles = n_e(files)

sin = read_ncdf(files[0],'sin')
ts = read_ncdf(files[0],'timestampUDP')
grt1 = grt_filter(tempconvert(sin[10+9*24,*],'grt29177','log'))
grt2 = grt_filter(tempconvert(sin[11+9*24,*],'grt29178','log'))
jd = parse_udp_timestamp(reform(ts[*,0,*]))
nt = n_e(jd)
help,jd,grt1
...
; Sometimes, for God knows what reason, there's no timestamp?!
        if (ts[0] ne -1) then begin
            tc = tempconvert(sin[10,*],'grt29177','log')
; Throw out crappy data
            if (min(tc) gt 0) then begin
                grt1_tmp = $
                  grt_filter(tempconvert(sin[10+9*24,*],'grt29177','log'))
                grt2_tmp = $
                  grt_filter(tempconvert(sin[11+9*24,*],'grt29178','log'))
                jd_tmp = parse_udp_timestamp(reform(ts[*,0,*]))
                nt = n_e(jd_tmp)
...
files = [file_search('~/data/telescope_tests/*/*board09.nc'),$
         file_search('/data/*/*board09.nc')]
