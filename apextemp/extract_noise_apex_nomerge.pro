...
;
; Hacked to deal with files that won't merge, 06Nov10, TC

pro extract_noise_apex_nomerge, file_root, yrange = yrange, navg = navg, sav_file=sav_file, ps_file=ps_file
...
; These will be 240 by ntime
ntod_all = lonarr(10)
for i=0,9 do begin
    istr = strcompress('0'+string(i),/rem)
    thisfile = file_root+'_board'+istr+'.nc'
    cos_in_temp = read_ncdf(thisfile,'cos')
    ntod_all[i] = n_e(cos_in_temp[0,*])    
endfor
ntod = min(ntod_all)
cos_in = fltarr(240,ntod)
sin_in = fltarr(240,ntod)
for i=0,9 do begin
    istr = strcompress('0'+string(i),/rem)
    thisfile = file_root+'_board'+istr+'.nc'
    cos_in_1 = read_ncdf(thisfile,'cos')
    cos_in[24*i:24*(i+1)-1,*] = cos_in_1[*,0L:ntod-1]
    sin_in_1 = read_ncdf(thisfile,'sin')
    sin_in[24*i:24*(i+1)-1,0L:*] = sin_in_1[*,0L:ntod-1]
endfor
; Get the timestamp and turn into jd
timestamp =  read_ncdf(thisfile,'timestampUDP')
jd = parse_udp_timestamp(timestamp)
sample_interval = median(fin_diff(jd)*24.*3600.d)
...
if n_elements(ps_file) eq 0 then ps_file = file_root + '_nomerge.ps'
if n_elements(sav_file) eq 0 then sav_file = file_root + '_nomerge.sav'
...
vchannel_psd_optical_deglitch = dblarr(160,navg/2+1)

for f = 0,160-1 do begin

    vchannel_psd_optical[f,*] = (psd(optical[f,*]/gain,$
                                     samp=sample_interval,avg=navg)).psd
;    thisopt_deglitch = deglitch(reform(optical[f,*]),step=1.d-4,sigma=20.,/quiet)
; I do NOT get what that deglitching routine is doing.  substitute a
; quick & dirty one that I understand
    thisopt_deglitch = deglitch_quick_tc(reform(optical[f,*]),nsigma=5.)
    optical_deglitch[f,*] = thisopt_deglitch
    vchannel_psd_optical_deglitch[f,*] = (psd(thisopt_deglitch/gain,$
                                              samp=sample_interval,avg=navg)).psd
...
avgpsd_deglitch = average_psds(vchannel_psd_optical_deglitch,flags,/median)
...
save,vbias,vbias_deglitch,vchannel,vchannel_deglitch,$
  ...
  optical_deglitch, vchannel_psd_optical_deglitch, avgpsd_deglitch, $
  file=sav_file
