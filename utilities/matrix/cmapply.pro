; $Id: cmapply.pro,v 1.2 2003/10/23 21:11:51 observer Exp $ 
; $Log: cmapply.pro,v $
; Revision 1.2  2003/10/23 21:11:51  observer
; *** empty log message ***
;
;
;+
...
;   performance reasons.
;
;   The possible functions are:             (and number of loop
;   iterations:)
;     +     - Performs a sum (as in TOTAL)       number of collapsed
;             dimensions
;     AND   - Finds LOGICAL "AND" (not bitwise)  same
;     OR    - Finds LOGICAL "OR"  (not bitwise)  same
;     *     - Performs a product                 LOG_2[no. of
;             collapsed elts.]
;
;     MIN   - Finds the minimum value            smaller of no. of
;             collapsed
;     MAX   - Finds the maximum value            or output elements
;
;     USER  - Applies user-defined function      no. of output
;             elements
;
;
...
;               using common blocks.
;               DEFAULT: undefined (i.e., no keywords passed by
;               _EXTRA)
;
; RETURN VALUE:
;
