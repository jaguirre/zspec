...
;Original file (make_fitcal_wrapper) Created 09/06/10 by KSS.
; 09/06/10 - KSS - Fixed minor bugs with count # of observations and
;            array indices.
;                - Fixed so that correction for flagged channels is 1.0
;Modified file, make_fitcal_wrapper_apex, by JRK 10/21/10
; 12/19/2012 - KSS - Committed latest revisions to svn

pro make_fitcal_wrapper_apex, coadd_list, at_telescope=at_telescope, $
                         plot_obs=plot_obs, $
...
if not keyword_set(skip_uber_spectrum) then begin
    ...
    for i = 0, nobs-1 do begin
        uber_spectrum, coadd_list, $
          listnum=i+1, /dateobs_format, /apex
    endfor
   ...
   if keyword_set(plot_obs) then begin
        for i = 0, nobs-1 do begin
            ...
            ;stop
            plot_uber_spectrum_ks, spectra_dir+'/'+sftemp, pftemp, custom_title=pftemp
        endfor
    endif
...
if not keyword_set(skip_fit) then begin
    ...
    for i = 0, nobs-1 do begin
        ...
        use=where(finite(spec) and finite(err),complement=nofin)
        if nofin(0) ne -1 then flags(nofin)=0
...
for j = 0, nbolo-1 do begin
    ...
    wfin=where(finite(relfac_all(j,*)),complement=nofin)
    if nofin(0) ne -1 then obs_flag(nofin)=0
    whgood = where(obs_flag eq 1, n)
    if n eq 0 then begin
        mean_relfac[j]=0;!values.F_NAN
        std_relfac[j]=0;!values.F_NAN
    endif else begin
...
for i = 0, nobs-1 do $
  oplot, indgen(nbolo), relfac_all[*,i], psym=1
oplot, indgen(nbolo), mean_relfac, psym=7, color=250
