...
function discardNods2, structure
...
for alpha=0, n-1 do begin
    ...
    curStruct=structure.(names[alpha])
    ...
    for j=0,2 do begin          ;Algorithm loops 3 times
        ...
        for i=0, s[1]-1 do begin
            ...
            f=where(finite(nodspec[i,*]))
            if f[0] eq -1 then begin
                bad=1
                break
            endif

            numGood=n_elements(where(mask[i,*] eq 1 and finite(nodspec)))
            avg[i]=total(nodspec[i,f]*mask[i,*])/numGood
            av2=total(nodspec[i,f]^2*mask[i,*])/numGood
