function MS_decorrscript, vopt, period,coeffs=coeffs, timestreams=timestreams, fits=fits
old=vopt
n_bolos=n_e(vopt)
...
fits=dblarr(n_bolos, n_nods, n_pos, n_time)
f=findgen(n_time)
weights=replicate(1, n_time)

for bolo=0, n_bolos-1 do $
  for nod=0, n_nods-1 do $
...
for nod=0, n_nods-1 do begin
    for pos=0, n_pos-1 do begin
        ...
        for time=0, n_time-1 do begin
            ...
            sum=0
            for bolo=15, (n_bolos-15)/2+15 do $
              sum+=(vopt[bolo].nod[nod].pos[pos].time[time]-$
                    mean(vopt[bolo].nod[nod].pos[pos].time))
            
            a1[time]=sum/((n_bolos-15)/2)

            sum=0
            for bolo=(n_bolos-15)/2+16, n_bolos-1 do $
              sum+=(vopt[bolo].nod[nod].pos[pos].time[time]-$
                    mean(vopt[bolo].nod[nod].pos[pos].time)) 
            
            a2[time]=sum/((n_bolos-15)/2)
        endfor
        
        a1-=mean(a1)
        a2-=mean(a2)

        for bolo=15, (n_bolos-15)/2+15 do begin
      ...
      for bolo=(n_bolos-15)/2+16, n_bolos-1 do begin
          m=mean(vopt[bolo].nod[nod].pos[pos].time)
