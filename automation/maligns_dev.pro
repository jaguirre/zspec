...
pro maligns_dev, date, obs_num, new = new, plot = plot, onebze = onebze
...
;; OK.  If the read was timed out, the sequence number will be -1.  So
;; look for that.
;wh0 = where(seq_num[0,*] ne -1)
;wh1 = where(seq_num[1,*] ne -1)
;wh2 = where(seq_num[2,*] ne -1)
;wh3 = where(seq_num[3,*] ne -1)
;wh4 = where(seq_num[4,*] ne -1)
;wh5 = where(seq_num[5,*] ne -1)
;wh6 = where(seq_num[6,*] ne -1)
;wh7 = where(seq_num[7,*] ne -1)
;wh8 = where(seq_num[8,*] ne -1)
;wh9 = where(seq_num[9,*] ne -1)
;
;; Find the miminum sequence number.
;minseq = min([seq_num[0,wh0[0]], $
;              seq_num[1,wh1[0]], $
;              seq_num[2,wh2[0]], $
;              seq_num[3,wh3[0]], $
;              seq_num[4,wh4[0]], $
