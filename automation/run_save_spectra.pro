...
pro run_save_spectra, rel_phase=rel_phase, cont=cont, obs_list=obs_list,$
                      obsnum_start=obsnum_start,$
                      obsnum_end=obsnum_end,date=date, $
                      no_phase_file=no_phase_file, $
                      _EXTRA = EXTRA

;check for inconsistencies
if keyword_set(no_phase_file) and not keyword_set(obs_list) then message, 'ERROR! If setting NO_PHASE_FILE, must also set OBS_LIST. Quitting...'
...
case 1 of 
    keyword_set(cont): begin
        ...
        while (carry_on) do begin
            ...
            if file_search(this_obs_file) ne '' then begin
                ...
                mogul=save_spectra(year,month,night,obsnum,rel_phase,$
                                   /deglitch, _EXTRA = EXTRA)
                if obsnum eq obs_end then begin
                    print,'Demodulated observations '+strcompress(obsnum_start,/rem)+$
    ...
    keyword_set(obs_list): begin
        ...
        if ~keyword_set(obsnum_start) then obs_start=0 else obs_start=obsnum_start
          ...
          for i=obs_start,n_obs-1 do begin
              if flag[i] eq 1 then begin
                  ...
                  if maligned eq '' then begin
                      ...
                      demod:
                      if keyword_set(no_phase_file) then begin
                          mogul=save_spectra(year[i],month[i],night[i],obs[i],$
                                             1,$                     
                                             /deglitch, _EXTRA = EXTRA)
                      endif else begin
                          mogul=save_spectra(year[i],month[i],night[i],obs[i],$
