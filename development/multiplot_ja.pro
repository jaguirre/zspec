...
;-
pro multiplot_ja,help=help,pmulti $
  ,initialize=initialize,reset=reset,default=default $
  ,rowmajor=rowmajor,verbose=verbose, xscale_margins = xscale_margins, $
              yscale_margins = yscale_margins
;
;	=====>> COMMON
;
common multiplot $
...
; I'm hacking this up to be able to adjust the margins ... setting the
; global variables doesn't seem to do what I want.
if (keyword_set(xscale_margins)) then xscale_margins = xscale_margins $
  else xscale_margins = [1.,1.]
if (keyword_set(yscale_margins)) then yscale_margins = yscale_margins $
  else yscale_margins = [1.,1.]
; in normlized coordinates
  margins = [xscale_margins[0]*(min(!x.window)-min(!x.region)), $	
             yscale_margins[0]*(min(!y.window)-min(!y.region)), $
             xscale_margins[1]*(max(!x.region)-max(!x.window)), $
             yscale_margins[1]*(max(!y.region)-max(!y.window))]
  noerase = !p.noerase
