pro fivepoint_ja, filename,use_co=use_co
...
fwhm_difflim = 1.22 * (2.99d8/(nu*1.d9)) / 10.4 / !dtor * 3600.
...
; removed test for nod_strcut, which seems to be a holdover from Phil's code
IF (p_info.npoints NE 5) THEN BEGIN
...
; Get the FAZO and FZAO from the RPC data
fazo = median(rpc_params.azimuth_fixed_offset)
fzao = -median(rpc_params.elevation_fixed_offset)
;help,fazo,fzao
...
; Plot everything normalized to the center position.  Easier to use.
    plot,bolos_plot,snu_plot[*,0],/xst,$
...
;;    fwhmy[n]=2*sqrt(2*alog(2))*ypar[2]
    ...
    sp = [max(s2fit,whmx), x2fit[whmx], y2fit[whmx], 30./2.35, 30./2.35]
...
; Offsets
    plot,xvec[1:3],yz,xrange=[xmin,xmax],yrange=[ymin,ymax],$
      ...
      subtit = string('Fitted center: ',mean(x0),mean(y0),$
                      fazo+mean(x0),fzao-mean(y0),$
                      format='(A,F7.1,",",F7.1,"!C","FAZO",F7.1,"  ","FZAO",F7.1)'),$
      ymarg=[6,4]
