Pro	rdisplay, image0, $
		...
		overplot=overplot, $
		enforce=enforce
resolve_routine, 'rimgexp', /is_function
resolve_routine, 'imgscl', /is_function
...
;			Wrap=wrap, $
                        Log=log_scaling, Levels=l, MaskValue=maskvalue) + $
                          Byte(bot)
        ENDIF ELSE BEGIN 
           byte_im = bytscl(im, min=minval, max=maxval, top=top-bot);ImgScl(im, Min=minval, Max=maxval, Top=top-bot, $
	                            ;Log=log_scaling, Levels=l, MaskValue=maskvalue) + $
             				;Byte(bot)
        ENDELSE  
    EndIf Else Begin
